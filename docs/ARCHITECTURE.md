...
## Sprint 2 — Recuperação híbrida

A recuperação do chat passou a ser coordenada por `HybridRetriever`:
...
A busca lexical usa full-text search do PostgreSQL quando disponível e fallback controlado para `ILIKE` em ambientes alternativos. A busca vetorial continua usando `knowledge_chunks.embedding_v2`.

O reranker é determinístico, configurável e auditável. Ele combina:

- similaridade vetorial;
- correspondência lexical;
- metadados da fonte/chunk;
- sobreposição direta com a pergunta.

A saída do chat inclui telemetria de recuperação para facilitar diagnóstico e regressão futura.
