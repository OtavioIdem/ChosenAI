...
class LearningService:
    ...
    def _index_training_candidate(self, candidate: TrainingCandidate) -> None:
        ...
        embeddings = self._embed_batch_blocking(chunks)
    ...
    def _embed_batch_blocking(self, texts: list[str]) -> list[list[float]]:
        try:
            return asyncio.run(self.embedding_provider.embed_batch(texts))
        except EmbeddingProviderError as exc:
            raise HTTPException(
                status_code=503,
                detail="Não foi possível gerar embeddings para indexar o candidato aprovado.",
            ) from exc
